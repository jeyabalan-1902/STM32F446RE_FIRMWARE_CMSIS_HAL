...
uint8_t supported_commands[] = {
                               ...
                               BL_FLASH_ERASE,
                               BL_MEM_WRITE,
                               BL_READ_PAGE_STATUS}; // Changed from sector to page
    ...
    while(1)
    {
        ...
        switch(bl_rx_buffer[1])
        {
            ...
            case BL_MEM_READ:
                ...
                break;
            case BL_READ_PAGE_STATUS: // Changed from sector to page
                bootloader_handle_read_page_status(bl_rx_buffer);
                break;
            case BL_OTP_READ:
...
void jump_to_updated_application(uint32_t app_start_address)
{
    ...
    // STM32F103 doesn't have VTOR register, so we skip this line
...
{
    uint16_t status;
    printf("BL_DEBUG_MSG:bootloader_handle_read_page_protection_status\n");
...
{
    // STM32F103C8T6 doesn't have OTP memory
    printf("BL_DEBUG_MSG: OTP not available on STM32F103C8T6\n");
    bootloader_send_nack();
}

void bootloader_handle_mem_read(uint8_t *bl_rx_buffer)
{
    // Implementation for memory read command
    // Can be implemented similar to F446RE version
}
...
void bootloader_handle_dis_rw_protect(uint8_t *bl_rx_buffer)
{
    ...
    if (!bootloader_verify_crc(&bl_rx_buffer[0], command_packet_len-4, host_crc))
    {
        ...
        printf("BL_DEBUG_MSG: flash protection disable status: %#x\n", status);
...
void bootloader_handle_endis_rw_protect(uint8_t *bl_rx_buffer)
{
    ...
    if (!bootloader_verify_crc(&bl_rx_buffer[0], command_packet_len-4, host_crc))
    {
        ...
        uint16_t page_mask = (bl_rx_buffer[3] << 8) | bl_rx_buffer[2];
        status = configure_flash_page_rw_protection(page_mask, bl_rx_buffer[4], 0);

        printf("BL_DEBUG_MSG: flash protection status: %#x\n", status);
        bootloader_uart_write_data(&status, 1);
    }
    else
    {
...
void bootloader_handle_flash_erase_cmd(uint8_t *bl_rx_buffer)
{
    ...
    if (!bootloader_verify_crc(&bl_rx_buffer[0], command_packet_len-4, host_crc))
    {
        printf("BL_DEBUG_MSG:checksum success !!\n");
        bootloader_send_ack(bl_rx_buffer[0], 1);
        printf("BL_DEBUG_MSG:initial_page : %d  no_of_pages: %d\n", bl_rx_buffer[2], bl_rx_buffer[3]);
...
uint8_t get_flash_rdp_level(void)
{
    ...
    // For STM32F103C8T6, read from Option Bytes
    volatile uint16_t *pOB_RDP = (uint16_t*) 0x1FFFF800;
    rdp_status = (uint8_t)(*pOB_RDP & 0xFF);
...
uint8_t execute_flash_erase(uint8_t page_number, uint8_t number_of_pages)
{
    ...
    // STM32F103C8T6 has 64 pages (0-63)
    if(number_of_pages > FLASH_PAGES_COUNT)
        return INVALID_PAGE;

    if((page_number == 0xff) || (page_number < FLASH_PAGES_COUNT))
    {
        ...
        {
            // Page erase
            uint8_t remaining_pages = FLASH_PAGES_COUNT - page_number;
            if(number_of_pages > remaining_pages)
            {
                number_of_pages = remaining_pages;
            }
            flashErase_handle.TypeErase = FLASH_TYPEERASE_PAGES;
            flashErase_handle.PageAddress = FLASH_BASE + (page_number * FLASH_PAGE_SIZE);
...
uint8_t execute_mem_write(uint8_t *pBuffer, uint32_t mem_address, uint32_t len)
{
    ...
    // STM32F103C8T6 programs in halfwords (16-bit)
    for(uint32_t i = 0; i < len; i += 2)
    {
        uint16_t halfword_data;
        if(i + 1 < len)
        {
            halfword_data = pBuffer[i] | (pBuffer[i + 1] << 8);
        }
        else
        {
            halfword_data = pBuffer[i] | 0xFF00; // Pad with 0xFF if odd number of bytes
        }

        status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, mem_address + i, halfword_data);
        if(status != HAL_OK)
        {
            break;
        }
