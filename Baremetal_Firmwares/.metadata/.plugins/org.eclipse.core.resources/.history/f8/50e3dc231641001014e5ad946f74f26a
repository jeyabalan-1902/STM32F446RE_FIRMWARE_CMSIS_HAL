...
{
    uint16_t smileRadiusX = radius / 2;
    uint16_t smileRadiusY = radius / 6;

    uint16_t mouthCenterY = centerY + radius / 3; // aligned with neutral mouth

    for (float angle = -60; angle <= 60; angle += 1.0f)
    {
        float rad = angle * 3.14159f / 180.0f;
        int x = centerX + (int)(smileRadiusX * cosf(rad));
        int y = mouthCenterY + (int)(smileRadiusY * sinf(rad));
...
void ILI9341_DrawSmileEmoji(uint16_t centerX, uint16_t centerY, uint16_t radius, uint16_t faceColor, uint16_t eyeColor, uint16_t mouthColor)
{
    ...
    uint16_t eyeOffsetX = radius / 2;
    uint16_t eyeOffsetY = radius / 2;
    ILI9341_DrawFilledCircle(centerX - eyeOffsetX, centerY - eyeOffsetY, eyeRadius, eyeColor); // Left eye
    ILI9341_DrawFilledCircle(centerX + eyeOffsetX, centerY - eyeOffsetY, eyeRadius, eyeColor); // Right eye

    // Draw smile (arc approximation using lines)
    uint16_t smileRadius = radius / 2;
    for (int angle = -60; angle <= 60; angle += 2)
    {
        float rad = angle * 3.14159f / 180.0f;
        int x1 = centerX + (smileRadius - 1) * cosf(rad);
        int y1 = centerY + (smileRadius / 2) * sinf(rad) + radius / 4;
        ILI9341_DrawPixel(x1, y1, mouthColor);
    }
}
